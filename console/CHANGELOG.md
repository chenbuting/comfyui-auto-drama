...
## 2026-08-31 · v0.13.40 — 第 1 步：类型可自填、时长按秒、横竖屏真正生效

### 本次更新内容

- 前端：类型改成输入框，可自己写，也可点常用词
- 前端：总时长改成按秒填写，可点 30/60/90/120 秒
- 前端：画幅竖版/横版会传到出视频和分镜提示词（不再写死竖屏）
- 前端：按钮改为「保存项目 / 清空重填」；项目库进度去掉 7/7
- 后端：剧本生成带上类型和总秒数；工作流按项目画幅选 9:16 或 16:9

### 验证方式

- 第 1 步类型能手打「古装权谋」，时长能填 45
- 选横版后提交，云端任务分辨率应为 16:9
- 旧项目「1 分钟」打开后应变成 60 秒
...
## 2026-08-31 · v0.13.39 — 按项目隔离停止/状态；假排队 3 分钟标丢失；分镜失败写到页面

### 本次更新内容

- 后端：第 7 步状态、停止、从断点继续只作用于**当前项目**；停止不再清空整队、不再误停别的项目
- 后端：链式推进按项目暂停；找上一段时不再误用别的片子的任务
- 后端：云端队列和记录都没有时，约 3 分钟标 `F-LOST`，不再空等 40 分钟
- 后端/前端：衔接分镜没画成时，第 7 步卡片和提示条直接写出来
- 启动：不再写死本机 ffmpeg 路径，优先用 PATH / `FFMPEG_BIN`
- 文档：重写 `console/README.md` 对齐 8 步；旧介绍片合成脚本标「勿用」；gitignore 补 `chain.lock` 等临时文件

### 验证方式

- 打开「巷口递刀」，第 7 步不应再看到「雨夜青石巷」卡片
- 停止后，只当前项目变「已停止」；若另有项目在跑应继续
- 人为让任务号从队列消失：约 3 分钟应变失败 `F-LOST`
- 衔接分镜 400 时，等待卡片应出现红字「衔接分镜没画成」
- 朋友电脑未装 `E:\AIDATA\tools\ffmpeg\...` 时，只要 PATH 里有 ffmpeg 就能抽帧

### 影响与注意事项

- 需重启控制台（`python start_daemons.py`）并 Ctrl+F5
- 以前点过「全局停止」的旧数据：再点一次「从断点继续」即可
- 成片请用控制台第 8 步，不要跑 `workflows/assemble.py`

### 失败码 / 错误提示变化

- `F-LOST`：队列和记录都没有时约 3 分钟后出现（不再等超时上限）
- 新增页面文案：`衔接分镜没画成，约 45 秒后再试（不会用旧分镜）`

---

## 2026-08-31 · v0.13.38 — 第 7 步刷新不再卡住，任务卡片能马上出来

### 本次更新内容

- 后端：`/api/status` 只查状态和下载，不再顺便重生分镜、提交下一段
- 守护进程：对账之后再单独推进链条
- 避免刷新第 7 步时页面一直停在「提交后这里显示状态」

### 验证方式

- 打开第 7 步或点刷新，几秒内应看到已有任务卡片（01–03 已完成等）
- 续接下一段仍由守护进程或「从断点继续」负责

### 影响与注意事项

- 需重启控制台
- 若点过停止，04 会显示已停止，需再点「从断点继续」

---

## 2026-08-31 · v0.13.37 — 从断点继续不再清掉已交云端的任务号

### 本次更新内容

- 后端：点「从断点继续」时，后面几段如果已经有任务号且云端还在（排队/生成/已完成），保留任务号，不再清空
- 避免 03 已经在云端做出片，本地却变成「等待上段」，刷新也变不成已完成
- 后端：状态查询时若总 history 列表没有该任务号，再按任务号单独查一次

### 验证方式

- 03 已交云端后立刻再点从断点继续：03 任务号应还在，云端做完后应变已完成并下载
- 云端已无任务、history 也空的旧号仍会被清掉，然后从上一完成段续接

### 影响与注意事项

- 需重启控制台
- 本次会把雨巷交刀_03 对回已完成的云端任务并下载，再尝试交 04
...
## 2026-08-31 · v0.13.36 — 链式一次只交下一段；衔接分镜没画成先不交视频

### 本次更新内容

- 后端：只允许紧邻上一段做完再交下一段（03 接 02），同一片子已有一段在跑就不再交后面的
- 从断点继续时，后面几段各自挂紧邻上一段，不再全部挂到已完成的那一段（避免 `02 → 04`）
- 后端：衔接分镜生图失败会重试 3 次；仍失败则本段先不交视频，约 45 秒后再试，不再偷偷用旧分镜
- 规则：分镜提示词再次写明只画一张整图，禁止四格

### 验证方式

- 续接后日志应是 `02 → 03`，不应出现 `02 → 04`
- 云端同一时刻只有下一段在排队/生成
- 生图 451/400 时日志应有「衔接分镜没画成，先不交视频」，该段保持等待

### 影响与注意事项

- 需重启控制台
- 视觉服务连不上不影响出片：看不懂末帧会用默认描述，只有生图失败才会卡住本段
...
## 2026-08-31 · v0.13.35 — 分镜「重新生成」按现有提示词重画图，不再改词

### 本次更新内容

- 第 5 步点某张分镜的「重新生成」：卡片里已有提示词就沿用，只重画出图
- 提示词为空时，才按勾选的「AI 按剧情生成」去写新词再画

### 影响与注意事项

- 刷新页面（Ctrl+F5）后生效
- 若既要改词又要重画，先改提示词框，或用「对话修改」

---

## 2026-08-31 · v0.13.34 — 分镜只画一张整图，不再画四格漫画

### 本次更新内容

- 后端 / 前端：衔接分镜和按剧情生分镜的提示词，改为只画一个瞬间
- 写明禁止四格、连环画、分格、拼图，避免第 2 段起被画成小格子条

### 验证方式

- 重启控制台并 Ctrl+F5 后，重新生成第 2 段及之后的分镜，应是一张完整画面
- 文件名仍可能带 `_bridge_`，但图里不应再是上下四格

### 影响与注意事项

- 已经生成的四格图不会自动变，需在第 5 步点「重新生成」
- 已合成的视频不会自动重做，要对齐新分镜需再提交生成

---

## 2026-08-30 · v0.13.33 — 链式一次只交下一段，不再用第1段同时开 02/03/04

### 本次更新内容

- 后端：只允许紧邻上一段做完再交下一段（02 接 01，03 接 02）
- 一次推进只提交一段；从断点继续时，后面几段不再全部挂到第 1 段末帧

### 验证方式

- 续接后云端队列同一时刻应只有下一段在跑，不应同时出现 02、03、04
- 日志应是 `01 → 02`，不应出现 `01 → 03`、`01 → 04`

### 影响与注意事项

- 需重启控制台
- 已经多交到云端的任务不会自动取消，用各段最新完成的即可

---

## 2026-08-30 · v0.13.32 — 点提交会清掉旧的「已停止」，01 做完后能接着交 02

### 本次更新内容

- 后端：提交成功后清掉之前留下的停止标记，并拉起链式守护进程
- 避免「只点了提交、没点停止」，第 1 段做完后后面几段一直等着不动

### 验证方式

- 若系统里还留着「已停止」，再点提交后第 7 步不应再显示停止红字
- 第 1 段完成后，第 2 段会自动进入排队/生成

### 影响与注意事项

- 需重启控制台后，下一次提交才带上这个修复
- 当前这批已手动续上第 2 段，网页停在第 7 步即可

---

## 2026-08-30 · v0.13.31 — 本地已下载视频不再黑屏，先显示封面

### 本次更新内容

- 后端：`/media/文件名?poster=1` 用 ffmpeg 抽一帧当封面
- 前端：已下载视频带封面图；第 7 步状态没变时不再整页重画，避免预览被刷成黑块

### 验证方式

- 重启控制台后刷新第 7 步，已完成卡片应先看到画面，点播放能出声画
- 请求 ` /media/一把伞_03_00001_.mp4?poster=1 ` 应返回 JPEG

### 影响与注意事项

- 需重启控制台并 Ctrl+F5
- 封面缓存在视频旁边（`xxx.mp4.poster.jpg`），不影响成片本身

---

## 2026-08-30 · v0.13.30 — 同一段不再重复提交，导入剧本能填进表格

### 本次更新内容

- 后端：这批还在生成或等待时，再点提交会直接拒绝；链式推进加锁，同段已交过或做完不再重交
- 后端：导入剧本 JSON 会带回 scene/对白/动作，第 2 步表格不再空白；读配置以磁盘为准，本机 8188 不再盖掉远程地址
- 前端：有段落在跑/等待时禁止再提交；界面改为纸色无渐变，标题为 Bruce Chen

### 验证方式

- 控制台已重启；连接测试能读到 config.json 里的远程 ComfyUI
- 导入「剧本 JSON」后表格有场景和对白；链式进行中再点提交会提示并拦截

### 影响与注意事项

- 需重启控制台并刷新页面（Ctrl+F5）
- 正在跑的这一批不用重交，等 03、04 自动接上即可

### 失败码 / 错误提示变化

- 新提示：「这批还在生成或等待上一段，请不要再点提交。等全部完成后再交。」
- 页面提示：「还有段落在生成或等待，请等这批全部完成后再提交」

---

## 2026-08-30 · v0.13.29 — 重生成第2段会接第1段末帧，不会误接旧第2段

### 本次更新内容

- 点某段「重新生成」并勾选链式时：用该段的紧邻上一段（例如 02 接 01），即使上一段已经接过旧版
- 不再因为「01 已经接过」就误接到旧的 02 结尾

### 影响与注意事项

- 需重启控制台并 Ctrl+F5
- 重生成 02 时勾选「链式衔接」，方式选「末帧 + 本段分镜」；旧 02 会保留，新的会加版本号

---

## 2026-08-30 · v0.13.28 — 链式第2段起视频不再带角色正脸

### 本次更新内容

- 画分镜仍用角色锚点图；第 1 段出视频仍带正脸锁开场
- 第 2 段起视频只给「上一段末帧 + 场景 + 本段分镜」，不再塞正脸大头照抢权重

### 影响与注意事项

- 需重启控制台并 Ctrl+F5
- 已在跑或已生成的段不会自动变；要重接需停止后再从断点继续（或重提第 2 段起）

---

## 2026-08-30 · v0.13.27 — 从断点继续只看当前项目，不再误判旧项目已完成

### 本次更新内容

- 「从断点继续」只续接当前打开的项目（例如巷口递刀 01 完成、02-04 暂停）
- 不再拿任务列表里版本号最高的旧项目（如雨夜青石巷 v3 已全部完成）来判断「无需继续」

### 影响与注意事项

- 需重启控制台；打开要对的项目后再点一次「从断点继续」

---

## 2026-08-30 · v0.13.26 — 第 4 步异步扩写也会把 12 秒动作表压进设定时长

### 本次更新内容

- 点「生成规范提示词」走后台扩写时：已有完整稿不再原样跳过，会按表格时长压缩 0-2…10-12 这类时间轴
- 新扩写的稿也会压进设定秒数；对白落在最后一两秒内
- 修了压缩后把「4.5-5 秒」误改成「4.5-6 秒」的问题

### 影响与注意事项

- 需重启控制台并 Ctrl+F5
- 旧卡片不会自己变，再点一次「生成规范提示词」即可（已有完整稿会很快对齐，不必重跑大模型）

---

## 2026-08-30 · v0.13.25 — 提示词动作时间轴按设定时长整体压缩

### 本次更新内容

- 重新生成提示词时：若正文按 12 秒排动作（0-2、2-4…10-12），会按你设的时长整段压缩（例如压成 6 秒），对白也会提前到段内
- 不再只改「12 秒」几个字、却留下超过设定的时间表

### 影响与注意事项

- 需重启控制台并 Ctrl+F5；改完时长后必须再点一次「生成规范提示词」

---

## 2026-08-30 · v0.13.24 — 重新生成提示词时按当前时长改写时间

### 本次更新内容

- 第 4 步再点「生成规范提示词」会先读剧本表里的时长，不再拿上次 10 秒旧正文
- 生成后自动把提示词里的「10 秒 / 0-10 秒」改成当前段秒数，并写上「本段时长 N 秒」

### 影响与注意事项

- 需重启控制台并 Ctrl+F5
- 改完第 2 步时长后，再生成一次提示词即可看到新时间

---

## 2026-08-30 · v0.13.23 — 新项目不再自动套用旧分镜，可手动选用

### 本次更新内容

- 打开/扫描素材时，不再按 `分镜_s1/s2` 把别的项目的分镜自动填进来
- 第 5 步每张分镜增加「选用已有图」下拉和「清除」，需要时再自己挑
- 已误填到「巷口递刀」的旧分镜会清掉，不影响旧项目自己保存的分镜

### 影响与注意事项

- Ctrl+F5 刷新即可；本项目分镜先空着，点生成或自己选图
- 旧项目「雨夜青石巷」里已经保存的分镜还在

---

## 2026-08-30 · v0.13.22 — 分镜对照角色锚点，视频同时锁人、接戏、对分镜

### 本次更新内容

- 画分镜时带上本段角色正面/正脸锚点图（第 2 段起再带上一张分镜）；锚点图本身不重画
- 第 2 段起出视频：参考图改为「上一段末帧 + 角色正脸 + 场景 + 本段分镜」，不再只给末帧和分镜两张
- 提交视频时角色只带正面/正脸，侧背不占坑，避免把分镜挤掉
- 多段没勾链式时会先确认，避免 10 段一起交导致接不上

### 影响与注意事项

- 需重启控制台并 Ctrl+F5
- **角色锚点、场景图不用动**；要对上人，请按顺序重画分镜 1～10
- 已在跑的旧任务不会自动变；先停止，再从断点继续

---

## 2026-08-30 · v0.13.21 — 链式用上一段真实末帧重生下一张分镜再出片

### 本次更新内容

- 上一段视频完成后：看真实末帧 → 重画下一张分镜（开头像末帧，后半才是本段剧情）→ 再提交「末帧 + 新分镜」
- 本地视觉 5 秒不通就改用云端看图，都不通仍按文字兜底去画，不把控制台卡死
- 新分镜文件名 `分镜_sN_bridge_*.png`，写入项目资产

### 影响与注意事项

- 需重启控制台；从断点继续后，下一段提交前会多等大约 1～2 分钟画衔接分镜
- 第 1 段不用重生；旧分镜原图保留，项目里该段会改指向新图

---

## 2026-08-30 · v0.13.20 — 分镜 2 起自动承接上一段结尾再画

### 本次更新内容

- 生成分镜图提示词时：第 2 段起会带上一段分镜/剧情结尾，要求开头接着画、不要另起海报
- 重新生成分镜 2～N 会重写提示词（避免沿用旧的独立海报提示）
- 一键生成缺失分镜也走同一套衔接逻辑

### 影响与注意事项

- 需重启控制台并 Ctrl+F5；**已有分镜 2～10 要按顺序重新生成图**
- 第 1 段分镜、角色图、场景图不用动
- 新分镜出来后再跑第 2 段及以后的视频

---

## 2026-08-30 · v0.13.19 — 链式每段都用「末帧 + 本段分镜」两张图

### 本次更新内容

- 第 2 段起每一段（3、4、5…）都走 R2V，只提交两张图：上一段末帧 + 本段分镜图
- 不再把角色四视图塞进链式段，避免抢戏导致分镜/接戏都失效
- 上一段已标记 chain_done 时，自动改接最近完成的前一段，避免第 3 段永远卡住

### 影响与注意事项

- 需重启控制台；已生成的旧段不会自动变，从断点继续或重跑第 2 段起才生效
- 第 1 段仍是角色/场景/分镜的完整 R2V

---

## 2026-08-30 · v0.13.18 — 链式自动重生衔接分镜

### 本次更新内容

- 勾选「I2V硬接 + 分镜」时：上一段完成后，用末帧画面描述 + 本段剧情，自动重生下一张分镜（开头接着上一段，后半才是本段戏）
- 新分镜写入项目资产（`分镜_sN_bridge_*.png`），视频仍用 I2V 硬接末帧，分镜只作过程参考
- 视觉服务或生图失败时沿用旧分镜提示，不中断链式出片

### 影响与注意事项

- 需重启控制台；本地视觉（8001）和 Boogu/文生图要能用，衔接分镜才会更新
- 旧分镜图不会删除，只是项目里该段指向新图
