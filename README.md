...
- 🚀 **批量生成**：T2V / I2V / R2V（多参考）模式；链式一次只交下一段（02 接 01，03 接 02）；第 2 段起视频参考为「上段末帧 + 场景 + 分镜」，不带角色正脸锚点；步数 4-50 可调（≤8 自动加速）
- 🧩 **分镜图**：只画一张完整瞬间，不画四格；「重新生成」有提示词时只重画、不改词
...
7. **提交生成**：模式 / 分辨率 / 步数 / 链式衔接 / 成片质量可选；状态、停止、从断点继续只作用于当前项目
8. **记录导出**：生成记录（版本批次）、失败码速查、CSV 导出、一键合成（请用这里，不要跑 `workflows/assemble.py`）
...
- **第 2 段不像上一段结尾**：链式第 2 段起不锁正脸锚点，跟的是上段末帧 + 本段分镜；分镜要先画成单张整图再提交视频
- **从断点继续没动**：确认当前项目、没点停止；链式一次只交下一段，上一段做完才会交下一段

## 协作开发

日常改 `dev`，不要直接推 `main`（`main` 已保护，必须走 PR）。

```bash
git clone https://github.com/chenbuting/comfyui-auto-drama.git
cd comfyui-auto-drama
git checkout dev
git pull origin dev
# 改文件后
git add .
...
合进稳定版：GitHub 开 Pull Request（`dev` → `main`），至少 1 人批准后合并。

仓库主人在 **Settings → Collaborators** 添加协作者，权限选 **Write**。不要提交 `config.json`（含密钥）。
